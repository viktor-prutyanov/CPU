...
{
    printf("tokens at %p, size = %d\n", this, tokens.size());
    printf("\t   i  Len               Str Instr        Num Type\n");
    size_t i = 0;
    std::for_each(tokens.begin(), tokens.end(), [&i](const Token *token) 
    {
        printf("\t[%3lu] %3lu {%16.*s}  %3d 0x%8X ", i, token->Len, token->Len, token->Str, token->Instr, token->Num);
        switch (token->Type)
        {
